...
<%
	String begDate; 	//开始日期[yyyyMMddHHmmss]
	String endDate; 	//结束日期[yyyyMMddHHmmss]
	int flag; 			//订单状态 0 所有 1 已支付 2 已撤销 3 部分退货 4退货处理中 5 全部退货 
	int begIndex; 		//起始索引号，可为NULL,默认填0
	String begOrder; 	//起始订单号，可为NULL
	String endOrder; 	//截至订单号，可为NULL
	int sortField ;		//排序字段1，订单号；2，金额；3，时间
	int sortOrder; 		//1：降序；2，升序
	String code; 
	String err ;
	String msg ;
	...
	if (ret != 0){  //初始化失败
		...
		BOCOMB2COPReply rep = client.queryHistoryOrder(begDate,endDate,flag,begIndex,begOrder,endOrder,sortField,sortOrder);
		if (rep == null){  
