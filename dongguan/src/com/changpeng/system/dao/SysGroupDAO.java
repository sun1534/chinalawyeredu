...
public class SysGroupDAO extends BasicDAO {
	...
	public void deleteLogic(final int groupid) {
		execute("update com.changpeng.models.system.SysGroup group set group.delflag=true where group.groupid=?", new Integer(groupid));
		Object object = getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) {
				String hql="update com.changpeng.models.system.SysGroup group set group.delflag=true where group.groupid=?";
...
//		this.update(group);
	...
	public List getChildGroup(int parentid) {
			...
			List list = find("from com.changpeng.models.system.SysGroup group where group.parentid=?", parentid);
			return list;
