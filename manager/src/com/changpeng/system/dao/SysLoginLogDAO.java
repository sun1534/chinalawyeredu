...
public class SysLoginLogDAO extends BasicDAO {
	...
	public void updateIsLast2False(int userid) {
		execute("update com.changpeng.models.SysLoginlog log set log.islast=false where log.userid=?", userid);
	}
	...
	public int getLoginCountByUserId(int userid) {
		List list = find("from com.changpeng.models.SysLoginlog log where log.userid=?", userid);
	...
	public Timestamp getLastLoginTime(int userid) {
		List list = find("from com.changpeng.models.SysLoginlog log where log.islast=true");
		if (list != null && list.size() != 0) {
