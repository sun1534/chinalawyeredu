...
public class ForumDeleteAction extends AbstractAction {
	...
	public String go() throws Exception {
		...
		if (deleteforum.getIsmain()) {
			...
			// 将所有的跟帖删除
			basicService
					.execute("delete from com.changpeng.models.Forum forum where forum.mainforum="
							+ mainforumid);
		} else {// 如果是跟帖，修改forumid为主题贴的id
			this.mainforumid = deleteforum.getMainforum();
			// 修改主题帖的回复数减1；
			Forum themainforum = (Forum) basicService.get(Forum.class,
					mainforumid);
			themainforum.setReplycount(themainforum.getReplycount() - 1);
			themainforum.setLastupdate(new java.sql.Timestamp(System
					.currentTimeMillis()));
			basicService.update(themainforum);

		}
		basicService.delete(deleteforum);
