	...
	public String requestService(org.dom4j.Element rootElement) {
		...
		try {
			...
			for (int i = 0; i < lawersize; i++) {
				...
				//有事务所并且不是离职的律师了
				if (lawyer.getSysGroup() != null&&lawyer.getSysGroup().getGroupid()!=-1) {
					lawyertemp.append("<lawyer>");
					lawyertemp.append("<userid>").append(lawyer.getUserid()).append("</userid>");
					lawyertemp.append("<cardno>").append(lawyer.getComments() == null ? "" : lawyer.	getComments()).append("</cardno>");
					...
					lawyertemp.append("<lawyerno>").append(lawyer.	getComments()).append("</lawyerno>");
					lawyertemp.append("<photo>").append(lawyer.getComments() == null ? "" : lawyer.getComments()).append("</photo>");
					lawyertemp.append("</lawyer>\r\n");
				}
				else {
