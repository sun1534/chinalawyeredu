...
public class GetLessonRequest extends ElearningRequests {
	...
	public String requestService(org.dom4j.Element rootElement) {
		...
		try {
			...
			detachedCriteria.add(Restrictions.eq("fromAddr",com.changpeng.common.Constants.FROMADDR));
			detachedCriteria.add(Restrictions.ne("onlineorlocal","online")); //不是在线课程
			...
			for (int i = 0; i < lessonsize; i++) {
				...
				lessontemp.append("<reqminute>").append(0).append("</reqminute>");
				lessontemp.append("</lesson>");
