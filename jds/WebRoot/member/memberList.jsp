...
function getDelete(){
     ...
     if (confirm("您确定要进行删除?")) {
          document.form1.action="memberDeletes.action";
          document.form1.submit();
          return true;
     }
     else {
  ...
  <TR>
    <TD height=30 bgColor=#FFFFFF>
		...
		<div align="center"><img src="../images/arr.gif" width="13" height="13"></div>
              ...
              <td width="97%"><span class="sort-title">人事管理&gt;&gt;人员列表</span></td>
            </tr>
          </table>
      </div></TD>
  </TR>
...
<s:form name="form1" action="memberDeletes.action" method="POST">
...
<s:iterator value="memberlist" status="status">
                      <TR class=listline>
                        <TD >
                        <INPUT type="checkbox" value='${memberid}' name="check">
                        </TD>
                     
                        <TD>${username}</TD>
                        <TD> <s:if test="statusid==1"> 正常 </s:if> <s:elseif  test="statusid==0"> 离职 </s:elseif > </TD>
                        <TD>${mobile}</TD>
						<TD>${email}</TD>
						<TD >
<s:if test="tmemResume==null"> 
  <a href="resumeCreate!input.action?memberid=${memberid}&pagenumber=${pagenumber}">添加</a>  
</s:if>  
<s:if test="tmemResume!=null">
  <a href="resumeView.action?resumeid=${memberid}&pagenumber=${pagenumber}">查看</a>
</s:if>  
						</TD>
                        <TD><a href="memberView.action?memberid=${memberid}&pagenumber=${pagenumber}">查看</a></TD>
                  </TR>
</s:iterator>
<s:if test="memberlist!=null">                    <TR bgcolor="#ECECFF" class="pt9-18">
...
<s:form name="pageForm" action="memberList.action" method="POST">
	<s:hidden name="pagenumber"/>
</s:form>
<s:form name="createForm" action="memberCreate!input.action" method="POST">
	<s:hidden name="pagenumber" value="${pagenumber}"/>
</s:form>
<s:form name="searchForm" action="memberSearch!input.action" method="POST">
