...
function getDelete(){
     ...
     if (confirm("您确定要进行删除?")) {
          document.form1.action="skillDeletes.action";
          document.form1.submit();
          return true;
     }
     else {
...
<s:form name="form1" action="skillDeletes.action" method="POST">
...
<s:iterator value="skilllist" status="status">
                      <TR class=listline>
                        <TD >
                        <INPUT type="checkbox" value='${skillid}' name="check">
                        </TD>
                        <TD>${skillname}</TD>
                        <TD>${degree}</TD>
                        <TD>${description}</TD>
                        <TD>${resumeid}</TD>
                        <TD>${years}</TD>
                        <TD><a href="skillView.action?skillid=${skillid}&pagenumber=${pagenumber}">查看</a></TD>
                  </TR>
</s:iterator>
<s:if test="skilllist!=null">                    <TR bgcolor="#ECECFF" class="pt9-18">
...
<s:form name="pageForm" action="skillList.action" method="POST">
	<s:hidden name="pagenumber"/>
</s:form>
<s:form name="createForm" action="skillCreate!input.action" method="POST">
	<s:hidden name="pagenumber" value="${pagenumber}"/>
</s:form>
<s:form name="searchForm" action="skillSearch!input.action" method="POST">
