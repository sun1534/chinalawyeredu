...
function getDelete(){
     ...
     if (confirm("您确定要进行删除?")) {
          document.form1.action="bankDeletes.action";
          document.form1.submit();
          return true;
     }
     else {
  ...
  <TR>
    <TD height=30 bgColor=#FFFFFF>
		...
		<div align="center"><img src="../images/arr.gif" width="13" height="13"></div>
              ...
              <td width="97%"><span class="sort-title">业务管理&gt;&gt;委托银行列表</span></td>
            </tr>
          </table>
      </div></TD>
  </TR>
...
<s:form name="form1" action="bankDeletes.action" method="POST">
...
<s:iterator value="banklist" status="status">
                      <TR class=listline>
                        <TD >
                        <INPUT type="checkbox" value='${bankid}' name="check">
                        </TD>
                      <TD >${bankname}</TD>
                      <TD >${contactpeople}</TD>
                      <TD >${contactphone}</TD>
                      <TD >${contactmobile}</TD>
                      <TD >${address}</TD>
                        <TD><a href="bankView.action?bankid=${bankid}&pagenumber=${pagenumber}">查看</a></TD>
                  </TR>
</s:iterator>
<s:if test="banklist!=null">                    <TR bgcolor="#ECECFF" class="pt9-18">
...
<s:form name="pageForm" action="bankList.action" method="POST">
	<s:hidden name="pagenumber"/>
</s:form>
<s:form name="createForm" action="bankCreate!input.action" method="POST">
	<s:hidden name="pagenumber" value="${pagenumber}"/>
</s:form>
<s:form name="searchForm" action="bankSearch!input.action" method="POST">
