...
<script type="text/javascript">
	...
	function saveall(flag){
		document.forms['setApproveForm'].elements['info.statusid'].value=flag;
		document.setApproveForm.submit();
		return true;
	}
</script>
</HEAD>
...
<s:if test="isedit==true">
                      ...
                      <INPUT name="approve1" type=submit class="mask" value="通过审批" onClick="javascript: saveall(3)">
                      <INPUT name="approve2" type=submit class="mask" value="不通过" onClick="javascript: saveall(4)">
	...
	<s:if test="isedit==true">
   <s:form name="editForm" action="infoApproveEdit!input.action" method="POST">
   ...
   <s:else>
      <s:form name="backForm" action="infoApproveList.action" method="POST">
      <s:hidden name="pagenumber" value="${pagenumber}"/>
      </s:form>
   </s:else>
   <s:form name="setApproveForm"  action="infoSetApprove.action" method="POST"  >
