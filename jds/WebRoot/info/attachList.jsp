...
function getDelete(){
     ...
     if (confirm("您确定要进行删除?")) {
          document.form1.action="attachDeletes.action";
          document.form1.submit();
          return true;
     }
     else {
  ...
  <TR>
    <TD height=30 bgColor=#FFFFFF>
		...
		<div align="center"><img src="../images/arr.gif" width="13" height="13"></div>
              ...
              <td width="97%"><span class="sort-title">信息管理&gt;&gt;附件列表</span></td>
            </tr>
          </table>
      </div></TD>
  </TR>
...
<s:form name="form1" action="attachDeletes.action" method="POST">
...
<s:iterator value="attachlist" status="status">
                      <TR class=listline>
                        <TD >
                        <INPUT type="checkbox" value='${attachid}' name="check">
                        </TD>
                        <TD>${infoid}</TD>
                        <TD>${filename}</TD>
                        <TD>${filepath}</TD>
                        <TD>${createuser}</TD>
                        <TD>${createtime}</TD>
                        <TD><a href="attachView.action?attachid=${attachid}&pagenumber=${pagenumber}">查看</a></TD>
                  </TR>
</s:iterator>
<s:if test="attachlist!=null">                    <TR bgcolor="#ECECFF" class="pt9-18">
...
<s:form name="pageForm" action="attachList.action" method="POST">
	<s:hidden name="pagenumber"/>
</s:form>
<s:form name="createForm" action="attachCreate!input.action" method="POST">
	<s:hidden name="pagenumber" value="${pagenumber}"/>
</s:form>
<s:form name="searchForm" action="attachSearch!input.action" method="POST">
