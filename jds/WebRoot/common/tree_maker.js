			...
			if(this.childCount==1);
				...
				if(line){
					...
					for(var i=line.length-1;i>=0;i--)
						line[i].innerHTML="<IMG  src="+tree.lineFolder+"/tree_I.png>";
				}
			}
			else {
...
function Tree_on_action(action)//fire when click node
{
	if(action.type==Tree_LINK)//link
	{
		...
		{
                     //window.open(action.link,target);
		 	var command = "parent."+target +".location = action.link";
...
function Tree_buildTree(node,treeView)//init treeView from array
{
	...
	{
		switch( node[index] ){
		case Tree_const_end:             //5
		  //  alert(index+"|Tree_const_end:"+node[index]);
			break;
		case Tree_const_begin_folder:	//-1		
		  //  alert(index+"|Tree_const_begin_folder:"+node[index]);
			count++;
			break;
		case Tree_const_end_folder:     //1
		   // alert(index+"|Tree_const_end_folder:"+node[index]);
			nodeParent=nodeParent.parent;
			count--;
			break;
		case Tree_const_file:			//2
		  //  alert(index+"|const_file:"+node[index]);
			...
			break;
		case Tree_const_folder:	        //0  
		   //alert(index+"|const_folder:"+node[index]);
	...
	//alert(Tree_const_end);
	//alert(Tree_const_begin_folder);
	//alert(Tree_const_end_folder);
	//alert(Tree_const_file);
	//alert(Tree_const_folder);
	treeView.flag=false;
	treeView.refresh();
}
//---------
...
function Tree_table(node)
{
	...
	if(tree.showLine){
		while(count<level){
			...
			else html+="<TD NAME=Tree_td_line_"+parent.id+" id=Tree_td_line_"+parent.id+"><IMG  src="+tree.lineFolder+"I.png></TD>";
		}
...
function Tree_GetLineImg(node)
{
	...
	if(node.childCount>0){
		if(node.expanded)
			return tree.lineFolder+((node.index==node.parent.childCount-1)?"Lminus.png":"Tminus.png");
		else
			return tree.lineFolder+((node.index==node.parent.childCount-1)?"Lplus.png":"Tplus.png");
	}
	else return tree.lineFolder+((node.index==node.parent.childCount-1)?"L.png":"T.png");
}