...
function getDelete(){
     ...
     if (confirm("您确定要进行删除?")) {
          document.form1.action="businessDeletes.action";
          document.form1.submit();
          return true;
     }
     else {
  ...
  <TR>
    <TD height=30 bgColor=#FFFFFF>
		...
		<div align="center"><img src="../images/arr.gif" width="13" height="13"></div>
              ...
              <td width="97%"><span class="sort-title">流程管理&gt;&gt;业务列表</span></td>
            </tr>
          </table>
      </div></TD>
  </TR>
...
<s:form name="form1" action="businessDeletes.action" method="POST">
                <TABLE width="100%"  border=0 align=center cellPadding=3 cellSpacing=1 bgcolor="#F9F9F7">
                  <TBODY>
                      <TR class="listheadline">
                        <TD>选择</TD>
                      <TD align=center>业务名</TD>
...
<s:iterator value="businesslist" status="status">
                      <TR class=listline>
                        <TD align=>
                        <INPUT type="checkbox" value='${businessid}' name="check">
                        </TD>
                        <TD>${businessname}</TD>
                        <TD>${usetype==1?"所有人适用":"成员适用"}</TD>
                        <TD>${module.modulename}</TD>
                        <TD align=center>
<s:if test="usetype!=1"> 
<a href="businessViewUsers.action?businessid=${businessid}">成员</a>
</s:if>   
						</TD>
                        <TD align=center>${statusid==1?"启用":"冻结"}</TD>
                        <TD align=center><a href="businessView.action?businessid=${businessid}&pagenumber=${pagenumber}">查看</a></TD>
                  </TR>
</s:iterator>
<s:if test="businesslist!=null">                    <TR bgcolor="#ECECFF" class="pt9-18">
...
<s:form name="pageForm" action="businessList.action" method="POST">
	<s:hidden name="pagenumber"/>
</s:form>
<s:form name="createForm" action="businessCreate!input.action" method="POST">
	<s:hidden name="pagenumber" value="${pagenumber}"/>
</s:form>
<s:form name="searchForm" action="businessSearch!input.action" method="POST">
