...
public class OperationUtil {
	...
	static{
		...
		/*listBank=com.changpeng.common.Globals.getInstance().findAll(" from ToprBank");
		bankMap=new HashMap<String,String>();
		System.out.println("..................");
		for(ToprBank bank:listBank){
			bankMap.put(bank.getBankid()+"", bank.getBankname());
		}*/
	}
	
	public static List<ToprBank> listBank(){
		//return com.changpeng.common.Globals.getInstance().findAll(" from ToprBank");
		return com.changpeng.common.CommonDatas.bankList;
	}
	public static HashMap<String,String> bankMap(){
		/*List<ToprBank> listBank=com.changpeng.common.Globals.getInstance().findAll(" from ToprBank");
	...
	public static HashMap<String,ToprCreditcard> creditcardMap(){
		...
		List<ToprCreditcard> listCreditcard=com.changpeng.common.Globals.getInstance().findAll(" from ToprCreditcard");
	...
	public static HashMap<String,ToprCreditcard> cardOfBank(long bankid,org.hibernate.Session session){
		...
		List<ToprCreditcard> list=session.createQuery(" from ToprCreditcard where repaystatus<>2 and bankid="+bankid).list();
	...
	public static String taskuser(long creditcardid){
		...
		try{
			con=globals.getCon();
			pstmt=con.prepareStatement("select a.username from tsys_user a,Topr_Credittask b where a.userid=b.userid and b.taskstat=0 and CREDITCARDID=?");
