...
public class MyCreditcardListAction extends AbstractListAction {
	...
	public String go() throws HibernateException {
		...
		/*
		 * Criteria criteria =
		 * getSession().createCriteria(ToprCredittask.class);
		 * criteria.add(Expression.eq("userid", curuser.getUserid()));
		 * if(paydate!=null&&!"".equals(paydate))
		 * criteria.add(Expression.eq("paydate", paydate)); //催收中
		 * criteria.add(Expression.eq("taskstat", 0));
		 * 
		 * Criteria c2=criteria.createCriteria("toprCreditcard"); if (bankid !=
		 * 0) c2.add(Expression.eq("bankid", bankid)); if (consigntype!=
		 * null&&!"".equals(consigntype)) c2.add(Expression.eq("consigntype",
		 * consigntype)); if (consignflag!= null&&!"".equals(consignflag))
		 * c2.add(Expression.eq("consignflag", consignflag)); if (username!=
		 * null&&!"".equals(username)) c2.add(Expression.like("username",
		 * username,MatchMode.ANYWHERE)); if (idcard!= null&&!"".equals(idcard))
		 * c2.add(Expression.like("idcard", idcard,MatchMode.ANYWHERE)); if
		 * (creditcard!= null&&!"".equals(creditcard))
		...
		tasklist = getQuery().setMaxResults(maxperpage).setFirstResult(maxperpage * pagenumber).setCacheable(true)
				.list();
		return SUCCESS;
	}
	...
	private Query getQuery() throws HibernateException {
		...
		else if (tdflag == 2) // 仅显示退单的,4是退单的
			queryName += "  and b.tdflag<>0";

		queryName += " and a.userid=" + curuser.getUserid();

		System.out.println("我的任务:"+queryName);
