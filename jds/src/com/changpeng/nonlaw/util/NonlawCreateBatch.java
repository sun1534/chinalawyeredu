...
public class NonlawCreateBatch {
		...
		try {
			...
			for (int i = sheet.getFirstRowNum() + 1; i <= sheet.getLastRowNum(); i++) {
				...
				if (row != null) {
					...
					if (/* cellnum<28|| */cellVal(row.getCell((short) 0)) == null
							|| cellVal(row.getCell((short) 1)) == null || cellVal(row.getCell((short) 2)) == null) {
						System.out.println("非诉的不处理数据：" + cellVal(row.getCell((short) 0)) + "==>"
								+ cellVal(row.getCell((short) 1)) + "==>" + cellVal(row.getCell((short) 2)));
						...
						nonlaw.setConsigndate(new java.text.SimpleDateFormat("yyyy-MM-dd").format(row
								.getCell((short) 0).getDateCellValue()));

						// System.out.println(new
						// java.text.SimpleDateFormat("yyyy-MM-dd").format(row.getCell((short)0).getDateCellValue()));
						...
						if (cellVal(row.getCell((short) 31)) != null && !"".equals(cellVal(row.getCell((short) 31)))) // 原委托日期
							// 对于新单，该值为空
							nonlaw.setOrisigndate(new java.text.SimpleDateFormat("yyyy-MM-dd").format(row.getCell(
									(short) 31).getDateCellValue()));
	 ...
	 */
		public static int addCustomerAndAddress(Session session, TsysUser sysUser, TusrCustomerNew customer,
			...
			if (nonlaw.getHomephoneold() != null && !"".equals(nonlaw.getHomephoneold().trim())) {
				...
				p.setUsername(nonlaw.getUsername() == null ? "" : nonlaw.getUsername());
				p.setHomeaddr(nonlaw.getHomeaddr() == null ? "" : nonlaw.getHomeaddr());
				p.setCompany(nonlaw.getCompany());
				p.setOprid(nonlaw.getNonlawid());
				p.setOprflag(2); // 非诉业务

				p.setComments("住宅电话");
				p.setPhone(phone(nonlaw.getHomephoneold() == null ? "" : nonlaw.getHomephoneold()));
			...
			if (nonlaw.getMobileold() != null && !"".equals(nonlaw.getMobileold().trim())) {
				...
				p.setComments("手机");
				p.setPhone(phone(nonlaw.getMobileold() == null ? "" : nonlaw.getMobileold()));
				p.setCreatetime(new java.util.Date());
				p.setCustomerid(customerid);
				String md5val = com.sxit.common.util.md5.MD5(p.getUsername() + p.getPhone() + p.getHomeaddr()
						+ p.getCustomerid());
			...
			if (nonlaw.getCompanyphone() != null && !"".equals(nonlaw.getCompanyphone().trim())) {
				...
				p.setComments("单位电话");
				p.setPhone(phone(nonlaw.getCompanyphone() == null ? "" : nonlaw.getCompanyphone()));
				p.setCreatetime(new java.util.Date());
				p.setCustomerid(customerid);
				String md5val = com.sxit.common.util.md5.MD5(p.getUsername() + p.getPhone() + p.getHomeaddr()
						+ p.getCustomerid());
	 ...
	 * @throws IOException
	 */
	public static List<TnlwNonlaw> save(File excel, String consigndate, long bankid, org.hibernate.Session session,
			List existCustomerList) throws IOException {
		...
		for (TnlwNonlaw nonlaw : nonlawList) {
			...
			if (nonlawMap.containsKey(nonlaw.getIdcard())) { // 账户重复，覆盖操作。
				...
				TusrCustomerNew customer = NewCustomerUtil.getCustomer(session, nonlaw.getUsername(), nonlaw
						.getIdcard());
				int customerid = addCustomerAndAddress(session, sysUser, customer, nonlaw);
			...
			for (int i = sheet.getFirstRowNum() + 1; i <= sheet.getLastRowNum(); i++) {
				...
				if (row != null) {
					...
					if (cellVal(row.getCell((short) 0)) != null) {
						...
						// long
						// nonlawid=Long.parseLong(cellVal(row.getCell((short)0)));
						// 第一列改为借据号
						...
						if (cell8Val != null && !"".equals(cell8Val))
							repaytime = new java.text.SimpleDateFormat("yyyy-MM-dd").format(row.getCell((short) 6)
									.getDateCellValue());
						...
						// 不是当月还款
						if (repaytime != null
								&& !repaytime.substring(0, 7).equals(
						...
						// System.out.println("update tnlw_nonlaw set
						// repaystatus="+repaystatus+",refee='"+curMonthFee+"'+'"+curfee+"'
						// where nonlawid='"+nonlawid+"'");
						stmt.addBatch("update tnlw_nonlaw set repaystatus=" + repaystatus + ",refee='" + curMonthFee
								+ "'+'" + curfee + "' where nonlawid='" + nonlawid + "'");

						if (repaystatus == 2)
							stmt.addBatch("update tnlw_nonlaw set curoverfee=0,curaccrualfee=0 where nonlawid='"
									+ nonlawid + "'");
	...
	private static String curMonthFee(Connection con, long nonlawid) throws SQLException {
		...
		try {
			pstmt = con
					.prepareStatement("select sum(fee) from Tnlw_Repaylog where substr(repaytime,0,7)=to_char(sysdate,'yyyy-mm') and nonlawid="
							+ nonlawid);
