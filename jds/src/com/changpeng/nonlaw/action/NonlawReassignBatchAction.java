...
public class NonlawReassignBatchAction extends AbstractAction {
	private File file;
	...
	public NonlawReassignBatchAction() {
	}
...
//			String toPath=ServletActionContext.getServletContext().getRealPath("")+extendPath;
			...
			try {
				...
				if(!ext.equalsIgnoreCase(".xls")){
					...
					nextpage="nonlawtaskList.action";
					return SUCCESS;
				}
			}catch(IOException e){
    ...
    public String input() throws Exception {
    	...
    	private void reassigntask(Connection con,long nonlawid,long userid) throws SQLException{
    		Statement stmt=null;
    		ResultSet rs=null;
    		try{
    			...
    			//更新任务状态为催收中
    			stmt.addBatch("update tnlw_nonlaw set state=1 where nonlawid="+nonlawid);
    			 //如果任务存在  需将原始催收任务更改为撤单
    			stmt.addBatch("update tnlw_nonlawtask set taskstat=1 where nonlawid="+nonlawid);
    			//插入用户任务表
    			stmt.addBatch("insert into tnlw_nonlawtask (nonlawtaskid,nonlawid,userid,createtime,taskstat) values (TnlwNonlawtaskid.nextval,"+nonlawid+","+userid+",sysdate,0)");
    			//将对应的客户信息记录同步到该用户通讯录
    			stmt.addBatch("update tusr_address set userid="+userid+" where oprid="+nonlawid+" and oprflag=2");
        		//将对应的日志记录更改到新接单人
    			stmt.addBatch("update tnlw_nonlawlog set userid="+userid+" where nonlawid="+nonlawid);
        		stmt.executeBatch();
    			con.commit();
    		}finally{
    			if(rs!=null) rs.close();
...
//    							long nonlawid=Long.parseLong(cellVal(row.getCell((short)0)));
    							...
    							String duebill=cellVal(row.getCell((short)0)).replace("?", "").trim();
    							long nonlawid=NonlawCreateBatch.duebill2nonlawid(con,duebill);
    							
    						
    							
    							Long userid=userMap.get(cellVal(row.getCell((short)1)).trim());
    							System.out.println(duebill+",,,,,nonlawid::::::::"+nonlawid+",,,,userid=="+userid);
    							if(userid!=null){
    								reassigntask(con, nonlawid, userid);
    							}else{
