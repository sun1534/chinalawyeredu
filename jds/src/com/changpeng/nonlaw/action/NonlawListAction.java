...
public class NonlawListAction extends AbstractListAction {
	...
	public String go() throws HibernateException {
		 ...
		 * 
		 * if(lawflag==3) criteria.add(Expression.eq("lawflag", lawflag)); if
		 * (bankid != 0) criteria.add(Expression.eq("bankid", bankid));
		 * 
		 * if (username!= null&&!"".equals(username))
		 * criteria.add(Expression.like("username",
		 * username,MatchMode.ANYWHERE)); if (idcard!= null&&!"".equals(idcard))
		 * criteria.add(Expression.like("idcard", idcard,MatchMode.ANYWHERE));
		 * if (state != -1) criteria.add(Expression.eq("state", state));
		 * 
		 * if (bankname!= null&&!"".equals(bankname))
		 * criteria.add(Expression.eq("bankname", bankname)); if (consigndate!=
		 * null&&!"".equals(consigndate))
		 * criteria.add(Expression.eq("consigndate", consigndate)); nonlawlist
		 * =page(criteria);
		 */
		nonlawlist = getQuery().setMaxResults(maxperpage).setFirstResult(maxperpage * pagenumber).list();
		return SUCCESS;
	}
	...
	private Query getQuery() throws HibernateException {
		 ...
		 * if(chengbanren!=null&&!"".equals(chengbanren)){ queryName="select a
		 * from TnlwNonlaw a,TnlwNonlawtask b,TsysUser c where a.repaystatus<>2
		 * and a.nonlawid=b.tnlwNonlaw.nonlawid and b.userid=c.userid and
		 * c.username like '%"+chengbanren.trim()+"%' and b.taskstat=0"; }
		 */

		// 显示全清记录
		String queryName = "from TnlwNonlaw a where 1=1";
		if (chengbanren != null && !"".equals(chengbanren)) {
			queryName = "from TnlwNonlaw a,TnlwNonlawtask b,TsysUser c where a.nonlawid=b.tnlwNonlaw.nonlawid and b.userid=c.userid and c.username like '%"
