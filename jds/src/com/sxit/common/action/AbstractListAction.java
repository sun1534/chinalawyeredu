...
public abstract class AbstractListAction extends AbstractAction {
	...
	protected BasicDao basicDao = new BasicDao();
...
//			pagenumber=1;
	...
	private void initLawcasePagestring() {
		pagestring = "";
		if (recordsize > 0) {
			// pagestring = "<TR bgcolor=\"#FEF7E9\" class=\"pt9-18\"><TD
			// colSpan=15 align=\"middle\"> ";
			// pagestring+="<div align=\"left\" bgcolor=\"#FEF7E9\"
			// style='float:left'><span class=\"pt9-18\">";
			// pagestring+="每页显示<select name='maxperpage'
			// onchange=\"changePagesize(this.value)\">";
			// pagestring+="<option value='10'>10</option><option
			...
			// value='45'>45</option><option value='50'>50</option>";
			...
			// pagestring+="第<font
			// color=red><b>"+(pagenumber+1)+"</b></font>页/";
			pagestring += "第<input type='text' size='3' id='pages' value='"
					+ (pagenumber)
					+ "' onkeypress=\"if(event.keyCode==13) page(this.value-1);\">页<a href=\"javascript:page(document.getElementById('pages').value-1)\"><font color=red><b>GO</b></a>";
			...
			if (pagesize == 1) {
				...
				if (pagenumber + 1 == 1) {
					...
					pagestring += "<a href=\"javascript:page(1)\">首页</a>";
				...
				if (pagenumber + 1 == pagesize) {
					...
					pagestring += "<a href=\"javascript:page(" + (pagesize) + ")\"> 末页</a>";
				}
			}
	...
	public List page(Criteria criteria) {
		BasicDao basciDao = new BasicDao();
		basciDao.setSession(getSession());
		PageSupport ps = basciDao.findPageOnPageNo(criteria, maxperpage, pagenumber);
		...
		LOG.debug(recordsize + ",,,," + pagenumber + ",,,," + ps.getItems());

		return ps.getItems();
		// pagesize = (recordsize - 1) / maxperpage + 1;

		// return criteria.setMaxResults(maxperpage).setFirstResult(maxperpage *
