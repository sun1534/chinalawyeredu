 ...
 * @修改：
 */
public class PositionSearchAction  extends AbstractAction {
  private List positionlist;
  private TsysPosition positionExample=new TsysPosition();
  private String flag=""; //flag为in的时候表述输入查询条件 flag为out的时候表述输出查询结果
  private String departmentname;
  public PositionSearchAction() {
    rights="sys4,1";
  }
  //忽略空字符串
  ...
  protected String go() throws HibernateException {
      Criteria criteria = getSession().createCriteria(TsysPosition.class)
          .add(Example.create(positionExample) //事例查询
               .enableLike() //模糊查询
...
//      }
      //positionlist = criteria.setMaxResults(maxperpage).list();//一次最多输出maxpaerpage条记录
      positionlist = criteria.list(); //将结果集一次输出
      return SUCCESS;
   }
   ...
   public TsysPosition getPositionExample() {
