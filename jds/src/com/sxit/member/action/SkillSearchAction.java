...
public class SkillSearchAction  extends AbstractAction {
  ...
  private TmemSkill skillExample=new TmemSkill();
  private String flag=""; //flag为in的时候表述输入查询条件 flag为out的时候表述输出查询结果
  //private String departmentname;
  public SkillSearchAction() {
    rights="mem3,1";
  }
  //忽略空字符串
  ...
  protected String go() throws HibernateException {
      Criteria criteria = getSession().createCriteria(TmemSkill.class)
          .add(Example.create(skillExample) //事例查询
...
//      }
      //skilllist = criteria.setMaxResults(maxperpage).list();//一次最多输出maxpaerpage条记录
      skilllist = criteria.list(); //将结果集一次输出
      return SUCCESS;
   }
   ...
   public TmemSkill getSkillExample() {
           return skillExample;
   }
