...
public class WorkexpSearchAction  extends AbstractAction {
  private List workexplist;
  private TmemWorkexp workexpExample=new TmemWorkexp();
  private String flag=""; //flag为in的时候表述输入查询条件 flag为out的时候表述输出查询结果
  //private String departmentname;
  public WorkexpSearchAction() {
    rights="mem5,1";
  }
  //忽略空字符串
  ...
  protected String go() throws HibernateException {
      Criteria criteria = getSession().createCriteria(TmemWorkexp.class)
          .add(Example.create(workexpExample) //事例查询
...
//      }
      //workexplist = criteria.setMaxResults(maxperpage).list();//一次最多输出maxpaerpage条记录
      workexplist = criteria.list(); //将结果集一次输出
      return SUCCESS;
   }
   ...
   public TmemWorkexp getWorkexpExample() {
           return workexpExample;
   }
