...
public class DirectionSearchAction  extends AbstractAction {
  private List directionlist;
  private TwflDirection directionExample=new TwflDirection();
  private String flag=""; //flag为in的时候表述输入查询条件 flag为out的时候表述输出查询结果
  //private String departmentname;
  public DirectionSearchAction() {
    rights="wfl1,1";
  }
  //忽略空字符串
  ...
  protected String go() throws HibernateException {
      Criteria criteria = getSession().createCriteria(TwflDirection.class)
          .add(Example.create(directionExample) //事例查询
               .enableLike() //模糊查询
...
//      }
      //directionlist = criteria.setMaxResults(maxperpage).list();//一次最多输出maxpaerpage条记录
      directionlist = criteria.list(); //将结果集一次输出
      return SUCCESS;
   }
   ...
   public TwflDirection getDirectionExample() {
