...
public class Test {
	...
	public static void t() throws Exception {
		InputStream stream = new FileInputStream("d:/aa.xls");
		...
		for (int i = sheet.getFirstRowNum() + 1; i <= sheet.getLastRowNum(); i++) {
			...
			if (row != null) {
				int cellnum = row.getLastCellNum(); // 最大列数
				for (int ii = 0; ii < cellnum; ii++) {
					HSSFCell cell = row.getCell(ii);
					System.out.print(com.sxit.common.HSSFCellToString.toString(cell)+" ");
				}
	...
	public static void parseXLS(File file) throws IOException {
		InputStream stream = new FileInputStream("d:/a.xls");
		HSSFWorkbook workbook = new HSSFWorkbook(stream);
		HSSFSheet sheet = workbook.getSheetAt(0); // 第一张工作表
		// 第一行为表头 不用解析
		for (int i = sheet.getFirstRowNum() + 1; i <= sheet.getLastRowNum(); i++) {
