...
public class SysUserDAO extends BasicDAO {
	...
	public int deleteUsers(final int[] userids) {
		Object object = getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) {
				String hqlDelete = "delete from com.sxit.models.system.SysUser user where user.userid in (:userids)";
	...
	public int deleteUsersLogic(final int[] userids) {
		Object object = getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) {
				String hqlDelete = "update com.sxit.models.system.SysUser user set user.delflag=true where user.userid in (:userids)";
				Query queryObject = session.createQuery(hqlDelete);
				Integer[] params = new Integer[userids.length];
				for (int i = 0; i < userids.length; i++) {
					params[i] = new Integer(userids[i]);
	...
	public int userLogin(String loginName, String password) {
		List list = find("from com.sxit.models.system.SysUser sysuser where sysuser.delflag=false and sysuser.loginname=?", loginName);
	...
	public SysUser getSysUserByLoginname(String loginname){
		...
		List list=this.find("from com.sxit.models.system.SysUser user where user.loginname='"+loginname+"'");
		if(list==null||list.size()==0)
			return null;
		else
	...
	public SysUser getSysUser(String loginname,int userid){
		...
		List list=this.find("from com.sxit.models.system.SysUser user where user.userid!="+userid+" and user.loginname='"+loginname+"'");
		if(list==null||list.size()==0)
			return null;
		else
