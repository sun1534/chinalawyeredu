...
|--------------------------------------------------*/
// Node object
function Node(id, pid, name, url, title, target, icon, iconOpen, open,checkedorno) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
//	this.callback=(callback&&(typeof callback== 'function')) ? callback:defaultCallback;
//	alert(this.callback);
//	callback('bbbbb');
};
// Adds a new node to the node array
dTree.prototype.add = function(id, pid, name, url, title, target, icon,
		iconOpen, open,checkedorno) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url, title,
			target, icon, iconOpen, open,checkedorno);
};
...
//				+ '\')" value="'+ node.id +"|"+node.name+"'" class="cx"/>';
	str += '<input type="checkbox" name="id" id="c'+this.parEnum+'" onClick="caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" value="' + node.id +"|"+node.name+'" class="cx" '+node.checkedorno+'/>';
...
function caBox(formId, regx) {
	...
	//alert(formId+",,,");
	//alert(document.getElementById(formId).elements);
		...
		for (var i = 0; i < form.elements.length; i++) {
			...
			if (element.name == "id" && element.type == 'checkbox') {
				...
				if (element.id.indexOf(regx) != -1) {
					newselright(element.value.split("|")[0],element.value.split("|")[1],true);
		...
		for (j = 0; j < regxArray.length; j++) {
			...
			if (document.getElementById(pDiv)){
				if(document.getElementById(pDiv).value&&document.getElementById(pDiv).value!=""){
			  	//abcd(document.getElementById(pDiv).value+"===========22222222true");
		      newselright(document.getElementById(pDiv).value.split("|")[0],document.getElementById(pDiv).value.split("|")[1],true);
			  }
		...
		//abcd(document.getElementById(regx).value+"===========false");
		// unChecked
		var form = document.getElementById(formId);
		var regxArray = regx.split("-");
		for (var i = 0; i < form.elements.length; i++) {
			...
			if (element.name == "id" && element.type == 'checkbox') {
				...
				if (element.id.indexOf(regx) != -1) {
					...
					//	abcd(element.value+"=====false");
				  newselright(element.value.split("|")[0],element.value.split("|")[1],false);
				}
			}
		}
		...
		for (j = 0; j < regxArray.length - 1; j++) {
			if (!isCheckedByRec(form, regx.substring(0, regx
				...
				if (document.getElementById(pDiv)){
					  ...
					  if(document.getElementById(pDiv).value&&document.getElementById(pDiv).value!="")
						  //abcd(document.getElementById(pDiv).value+"===false");
					    newselright(document.getElementById(pDiv).value.split("|")[0],document.getElementById(pDiv).value.split("|")[1],false);
					}
			}
		}
