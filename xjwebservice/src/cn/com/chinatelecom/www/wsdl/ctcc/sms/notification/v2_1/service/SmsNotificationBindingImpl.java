...
public class SmsNotificationBindingImpl implements
	 ...
	 * 
	 */
	public void notifySmsReception_old(java.lang.String registrationIdentifier,
		...
		if (message != null) {
			...
			try {
				...
				if (busno != null && !busno.equals("") && !OrderConstant.PROVINCE_FIRST.contains(first)) {
						...
						if (areaCode != null && !areaCode.equals("")
							...
							if (result == -1) {
								...
								if (b == 1) { // 上行的是修改区号信息
									...
									Sms.sendSms(sender, "您选择的车牌类型为" + OrderConstant.MEMBER_CHEPAI.get(bustype)
											+ "车，请回复您的车牌号码（英文字母+数字），如车牌号为新AC4102则回复AC4102．返回请按回复0", "mo", productId);
							...
							if (result == -1) {
								...
								if (a == 1) { // 上行的是修改区号信息
									...
									Sms.sendSms(sender, "您选择的车牌类型为" + orderService.getMemberChepaiTypeName(bustype)
											+ "车，请回复您的车牌号码（英文字母+数字），如车牌号为新AC4102则回复c#AC4102。", "mo", productId);
									...
									if (!s.equals("未知"))
										Sms.sendSms(sender, "您已成功开通汽车保姆体验业务，您的车牌是"
												+ orderService.getMemberChepaiTypeName(uo.getChepaileixing()) + "车"
												+ uo.getChepai()
