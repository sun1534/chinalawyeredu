...
function swfUploadLoaded() {
	...
	document.getElementById("btnUpload").onclick = function () { self.startUpload(); }; //上传前加入操作 
}
...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("你选择了太多的文件.\n" + (message === 0 ? "You have reached the upload limit." : "您一次可以选择" + (message > 1 ? "上传 " + message + " 个文件." : "一个文件")));
			return;
		}
