...
body {
	background:url(../images/LTBY_9010.gif) fixed;
	font-size:14px;
}
td {
...
.pic{ background:url(../images/pic.jpg) no-repeat; width:90px; height:22px; border:none; cursor:pointer}
.title{ line-height:40px; text-align:center}
.title2{ background:url(../images/1.gif) no-repeat; height:90px;margin-top:20px}
...
<script>
	...
	function checkbixu(){
		...
		var i=0;
		for(;i<obj.length;i++){
		var childs=obj[i].parentNode.childNodes;
		for(var j=0;j<childs.length;j++){
		  if(childs[j].name&&childs[j].name=="replys")
		  {
		    reply=childs[j];
		    break;
		  }
	...
	<body>
		<TABLE cellSpacing=1 cellPadding=1 width="980px" bgColor=#999999 border=0 align="center">
			<TR>
		...
		<input type="hidden" name="diaochaid" value="${diaocha.diaochaid}"/>
			...
			<TABLE width="95%" border=0 align=center cellPadding=3 cellSpacing=1  style="background:#FFF">
				<TBODY>
                <TR>
              <TD><H3 class="title2"></H3></TD>
            </TR>
					<TR>
						<TD height="81">
                        	<H2 class="title">
					...
					<TR id="logic${wentiid}">
						<TD>
							...
							<s:if test="isbixu"><span style="color:red">*</span></s:if>）
							<input type="hidden" name="bixu" value="${isbixu}">
							</s:if>
							<span id="desc" style="color:red"></span>
						</TD>
			...
			<s:if test="!hasJoin">
			<p align="center">
            	<input name="input" type="submit" class="pic" value="  ">
			</p>
			</s:if>
